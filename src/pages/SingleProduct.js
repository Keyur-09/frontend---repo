...
const SingleProduct = () => {no
  ...
  useEffect(() => {
    const productInCart = cartState?.find((item) => {
      return item.productId?._id === getProductId && item.color?._id === color;
    });
    setAlreadyAdded(!!productInCart);
  }, [cartState, color, getProductId]);
  ...
  const uploadCart = () => {
    ...
    if (alreadyAdded) {
      for (let i = 0; i < cartState.length; i++) {
        if (
          cartState[i]?.productId?._id === productState?._id &&
          cartState[i]?.color?._id === color
        ) {
          dispatch(
            updateCartProduct({
              cartItemId: cartState[i]?._id,
              quantity: parseInt(cartState[i]?.quantity) + parseInt(quantity),
            })
          );
          break;
        }
